...
class Template_Requester(comms.Cog):
    ...
    async def load_weather(self):
        ...
        if not self.bot.is_closed():
            ...
            if not self.active_weather:
                printc('[...]: CHECKING <> SERVICE AVAILABILITY')
                # self.token = json.load(open(path('rehasher', 'configuration', 'config.json')))['']
                async with aiohttp.ClientSession() as session:
                    async with session.get() as test_response:
                        if test_response.status == 200:
                            printc('[ ! ]: <> SERVICE AVAILABLE')
                            self.active_service = True
                        else:
                            raise ValueError(f'WARNING: <> SERVICE NOT AVAILABLE {test_response}')
...
def setup(bot):
    bot.add_cog(Template_Requester(bot))
