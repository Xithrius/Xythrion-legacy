    ...
    async def load_riot_script(self):
        ...
        if not self.bot.is_closed():
            ...
            self.headers = {
                ...
                "X-Riot-Token": json.load(open(path('SoftBot', 'configuration', 'config.json')))['riot'],
            ...
            if next(iter(response)) == 'status':
                printc(f'WARNING: RIOT ACCOUNT CANNOT BE ACTIVATED: {(response["status"]["message"]).upper()}: {response["status"]["status_code"]}')
